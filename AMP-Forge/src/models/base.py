...
class BaseVAE(LightningModule):
    ...
    def __init__(
        ...
        pretrained_encoder_path: str = "../esm2_model",
        latent_dim: int = 64,
        ...
        '''
        self.latent_encoder = GRULatentEncoder(
            input_dim=enc_dim,
            hidden_dim=enc_dim,
            latent_dim=latent_dim,
            num_layers=1,  # 可配置
            bidirectional=True  # 推荐开启
        )
        '''
        # Predictor
    ...
    def encode(self, x: List[str]):
        ...
        #z, mu, logsigma = self.latent_encoder(enc_out)
    ...
    def loss_function(self, pred_seq: torch.Tensor, target_seq: torch.Tensor,
        ...
        # MSE (predictor) loss
        mse_loss = self.mse_loss(pred_prop, target_prob) #当前版本不考虑predictor的训练
        ...
        return total_loss, kl_loss, recon_loss, mse_loss #修改前版本
    ...
    def model_step(self, batch):
        ...
        y = y.unsqueeze(1)
        dec_probs, pred_property, mu, logvar, seqs_ids = self.forward(x)#修改前版本
        #dec_probs, mu, logvar, seqs_ids = self.forward(x)

        loss, kl_loss, recon_loss, mse_loss = \
            self.loss_function(dec_probs, seqs_ids, pred_property, y, mu, logvar)
        #loss, kl_loss, recon_loss = \
        #    self.loss_function(dec_probs, seqs_ids, mu, logvar)

        return loss, kl_loss, recon_loss, mse_loss

    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        loss, kl_loss, recon_loss, mse_loss = self.model_step(batch) #修改前版本
        #loss, kl_loss, recon_loss = self.model_step(batch)

        # update and log metrics
        self.train_total_loss(loss)
        self.train_kl_loss(kl_loss)
        self.train_recon_loss(recon_loss)
        self.train_mse_loss(mse_loss) #不再有mse_loss
