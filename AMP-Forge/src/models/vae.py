...
class BaseVAEModel(nn.Module):
    ...
    def reconstruct_from_wt(self,
        ...
        seqs = self.generate_from_latent(input_seq,latent)
        return seqs
    ...
    def reconstruct_from_wt_glob(self,
        ...
        for wt_seqs in wt_seq_chunk:
            seqs = self.reconstruct_from_wt(input_seq,wt_seqs, scale, factor, i)
...
class GruVAEModel(BaseVAEModel):
    ...
    def forward(self, seqs: List[str]):
        ...
        #print("input_ids shape:", [len(seq) for seq in seqs])
        #print(self.cfg.max_len)
        # Decoder
    ...
    def generate_from_latent(self, input_seq, latents: List[Tensor] | Tensor) -> List[str]:
        def main_process(latent: Tensor):
            upsl_latent = self.upsampler(latent) if self.use_attn else None
            pred_tokens = self.decoder.generate_from_latent(input_seq, latent, upsl_latent).tolist()
